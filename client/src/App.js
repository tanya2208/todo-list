...
function App() {
  ...
  useEffect(() => {
    let colorsTmp = []
    axios.get('http://localhost:8080/colors').then(response => {
        setColors(response.data)
        colorsTmp = response.data
    }).catch(err => {
        console.log(err);
    });

    axios.get('http://localhost:8080/lists').then(response => {
      setLists(response.data.map(list => {
        list.color = colorsTmp.find(color => color._id === list.colorId).hex
        return list
      }))
    }).catch(err => {
        console.log(err);
  ...
  const handleRemove = (index) => {
    let newList = [...lists]
    let listToDelete = newList[index]
    axios.delete('http://localhost:8080/lists', { id: listToDelete._id }, {
      headers: {
